using AccessService.Application;
...
public sealed class ProblemDetailsExceptionHandler(ILogger<ProblemDetailsExceptionHandler> logger) : IExceptionHandler
{
    public async ValueTask<bool> TryHandleAsync(HttpContext httpContext, Exception exception, CancellationToken cancellationToken)
    {
        ...
        {
            AccessDomainException domainEx => (domainEx.HttpStatus, domainEx.ErrorCode, "Error de validación de negocio.", domainEx.Message),
        ...
        if (status == 0)
        {
            logger.LogError(exception, "Error no manejado en access-service.");
